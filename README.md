...
30페이지짜리 논문 PDF를 마주하면 솔직히 읽기 싫잖아요. kiwi-paper는 그 논문을 나무위키 스타일로 바꿔줍니다. 내용은 그대로고, 읽는 맛이 생깁니다.
...
PDF, URL, 텍스트 파일 다 됩니다. arXiv 논문이든 뉴스 기사든 블로그든 가리지 않습니다.

처리는 4단계로 돌아갑니다. 초안 만들고, 구조랑 유머를 다듬고, 번역체나 AI체를 사람 말투로 고치고, 마지막에 HTML로 뽑아냅니다. 이 중에서 3단계 휴머나이즈가 핵심인데, "~함으로써 이루어진다"같은 번역투 문장이나 "다양한", "효과적인"이 난무하는 기계적 나열을 잡아서 실제로 사람이 쓴 것처럼 고쳐줍니다.

나무위키 특유의 ~~취소선~~, 각주 드립, 여담도 당연히 들어갑니다.

인자 없이 부르면 대화형 모드로 들어가서 입력 소스, 출력 형식, 저장 경로를 하나씩 골라줍니다.
...
대화형 모드에서는 입력 소스, 출력 형식, 저장 경로를 하나씩 고릅니다. 기본값은 HTML 출력입니다.
...
번역투 문장, 기계적 나열, 과잉 수식어, 동어반복 같은 걸 잡아내는 3단계 휴머나이즈가 사실상 이 프로젝트의 존재 이유입니다.

## HTML 렌더링

4단계에서 나무위키 느낌의 HTML을 뽑습니다. 라이트/다크 모드는 시스템 설정을 따라가고, 스크롤해도 목차 사이드바가 따라옵니다. 수식이랑 코드 하이라이팅도 됩니다. 문서가 여러 개면 이전/다음 네비게이션이 생기고, 모바일 대응이랑 인쇄 최적화도 포함됩니다.
...
[`examples/`](examples/) 디렉토리에 변환된 예시가 있습니다:

- [`attention-is-all-you-need.md`](examples/attention-is-all-you-need.md) — "Attention Is All You Need" Transformer 논문

~~취소선~~ 유머, 각주, 여담, 수식, 표 등 나무위키 문서에서 볼 법한 요소가 전부 들어 있습니다.

## 호환성

Claude Code 외에도 주요 AI 도구는 대부분 지원합니다.
...
`./install.sh` 한 번이면 CLI 도구는 전부 됩니다. Cursor, Antigravity 같은 IDE는 프로젝트에 룰 파일을 복사해서 쓰고, Claude Cowork는 SKILL.md를 직접 읽어주면 됩니다.
