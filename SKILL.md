...
원본 문서의 그림, 사진, 다이어그램, 표, 수식 이미지 등 모든 시각 자료를 빠짐없이 포함합니다.

#### 입력 유형별 처리

| 입력 유형 | 처리 방식 |
|----------|----------|
| **URL (웹페이지)** | `<img>`, `<figure>`, `<picture>` 태그의 이미지를 자동 다운로드하여 `images/`에 저장 |
| **URL (마크다운)** | `![](url)` 형태의 원격 이미지를 자동 다운로드 |
| **로컬 마크다운** | 상대 경로 이미지(`./figures/fig1.png`)를 자동 복사 |
| **PDF** | `Read` 도구로 이미지를 확인하고, 마크다운에 설명과 함께 삽입 |
| **텍스트/기타** | 본문에 언급된 그림을 `WebSearch`로 찾아 링크 삽입 |

#### 이미지 캡션 규칙

나무위키 스타일 캡션을 반드시 작성합니다:
...
캡션 작성 가이드:
- `> **[그림 N]**` 형태로 번호 매기기
- 그림이 무엇을 보여주는지 한 줄로 설명
- 적절한 곳에 ~~취소선~~ 유머 추가
- 출처가 있으면 `(출처: [원문](URL))` 추가

#### 추출 불가능한 이미지

PDF에서 이미지 추출이 불가능하거나 원본이 없는 경우:
- 텍스트로 가능한 한 상세히 묘사
- ASCII 아트나 마크다운 표로 대체 가능한 경우 대체
- `> **[그림 N]** (원문 참조)` 형태로 원문 참조 안내
- `WebSearch`로 해당 그림의 공개 버전을 찾아보기

#### 기타 리소스

- **표/차트**: 가능하면 마크다운 표로 재구성, 복잡한 경우 이미지로 캡처
- **수식 이미지**: LaTeX로 재작성 (`$$...$$`), 원본 이미지는 보조 참고용
- **코드 스크린샷**: 텍스트 코드 블록으로 재작성 (```language ... ```)
- **동영상/GIF**: 썸네일 이미지 + 원본 링크 제공
- **슬라이드 그림**: 개별 슬라이드 이미지를 해당 섹션에 배치
