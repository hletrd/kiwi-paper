...
원본 문서에 그림, 사진, 다이어그램이 있으면 반드시 추출하여 포함합니다:

1. **URL 입력**: 웹페이지의 이미지를 자동 다운로드합니다. 렌더러가 `images/` 디렉토리에 저장합니다.
2. **PDF 입력**: `Read` 도구로 PDF의 이미지를 확인하고, 중요한 그림은 설명과 함께 마크다운에 삽입합니다.
   - 그림이 추출 불가능한 경우, 텍스트로 상세히 묘사하고 `> **[그림 N]** 설명` 형태로 캡션을 답니다.
3. **이미지 캡션**: 나무위키 스타일로 유머를 곁들인 캡션을 작성합니다.
   - 예: `> **[그림 1]** Transformer 아키텍처. ~~이 그림을 안 본 NLP 연구자는 없다~~`
4. **이미지 배치**: 관련 섹션 바로 아래에 배치하고, 본문에서 "[그림 N] 참조" 식으로 언급합니다.
5. **외부 이미지 링크**: 원본 이미지의 출처가 있으면 캡션에 링크를 포함합니다.

마크다운에서 이미지 삽입:
```markdown
![Transformer 아키텍처](images/img-01.png)
> **[그림 1]** Transformer의 전체 아키텍처. ~~이 그림 한 장이 1000편의 논문을 낳았다~~
```
...
- **이미지 자동 다운로드**: URL 소스의 이미지를 자동 추출, `images/` 디렉토리에 저장
