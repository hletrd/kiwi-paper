...
- **목차 사이드바**: 스크롤 추적, 모바일에서는 햄버거 메뉴
- **수식 렌더링**: KaTeX로 LaTeX 수식을 서버사이드 렌더링
- **코드 하이라이팅**: Shiki로 VS Code 수준의 구문 강조 (듀얼 테마)
- **각주 백링크**: 클릭 시 본문↔각주 이동
- **문서간 네비게이션**: 여러 파일 렌더링 시 prev/next 링크 + 인덱스
- **한국어 타이포그래피**: Pretendard 폰트, `word-break: keep-all`
