...
version: 4.0.0
description: |
  논문, 전공 서적, 기술 명세서, API 문서, 사용설명서 등 모든 종류의 기술 문서를
...
```
AskUserQuestion:
  question: "이 문서의 유형이 뭔가요?"
  header: "문서 유형"
  options:
    - label: "학술 논문"
      description: "학술 저널이나 컨퍼런스 논문 (arXiv, IEEE, ACM 등)"
    - label: "기술 명세서/스펙"
      description: "RFC, W3C 스펙, 프로토콜 명세, 표준 문서 등"
    - label: "API 문서"
      description: "REST API, SDK, 라이브러리 레퍼런스 등"
    - label: "사용설명서/가이드"
      description: "제품 매뉴얼, 튜토리얼, 설치 가이드, 운영 가이드 등"
```

**자동 감지 힌트**:
- arXiv, DOI, "Abstract", "Related Work" → 학술 논문
- "RFC", "MUST", "SHALL", "specification", 버전 번호 나열 → 기술 명세서
- HTTP 메서드, 엔드포인트, 요청/응답 예시, "Authentication" → API 문서
- "Getting Started", "Installation", "Quick Start", 스크린샷 → 사용설명서
...
- 코드 블록, 명령어, 설정값 안에는 유머를 넣지 않는다
- 코드 예제 *옆* 설명에서 유머를 넣는다
- 에러 메시지나 상태 코드 설명에 ~~취소선~~ 번역이 효과적이다
